...
public sealed interface Outgoing<T> extends Message<T> {
    ...
    /**
     * @return An {@link OutgoingMessageBuilder} initialized with the metadata and content of this message.
     */
    default OutgoingMessageBuilder<?, T> builder() {
        return new OutgoingMessageBuilder<>().setContent(this.content())
    ...
    record Response<REQ, RES>(AMQP.BasicProperties properties, RES content, Incoming.Request<REQ> request)
        ...
        @Override
        public OutgoingMessageBuilder<REQ, RES> builder() {
            return new OutgoingMessageBuilder<>(content, request).setProperties(this.properties());
        }
    }
}
