...
public interface MessageWriter<T> extends Prioritized {
    ...
    /**
     * Serializes the given message by writing into the given {@link OutgoingMessageBuilder}, in particular by writing
     * to its {@link OutgoingMessageBuilder#content() content} {@link OutputStream}. The builder will be initialized
     * with the same metadata as the {@code originalMessage} and an {@code OutputStream} that has not written any bytes
     * yet. The builder's {@link OutgoingMessageBuilder#propertiesBuilder() message properties} can also be manipulated,
     * e.g. by setting additional headers, setting the message's {@link BasicProperties#getContentType() content type}
     * and/or {@link BasicProperties#getType() message type}.
     * <p>
     * Attempts to change the {@link OutgoingMessageBuilder#setExchange(String) exchange} or
     * {@link OutgoingMessageBuilder#setRoutingKey(String) routing key} of the message should not be made and may indeed
     ...
     */
    void write(Outgoing<T> originalMessage, OutgoingMessageBuilder<?, OutputStream> serializedMessage)
            throws IOException;
}
